...
public class Room {
    ...
    public static Room readRoom(File f, GameEngine engine) {
        ...
        // Contadores para validar a presença de peixes
        int bigFishCount = 0;
        ...
        try (Scanner scanner = new Scanner(f)) {
                ...
                for (int x = 0; x < line.length(); x++) {
                    ...
                    switch (c) {
                        ...
                        case 'B':
                            bigFishCount++;
                            if (bigFishCount > 1) {
                                throw new IllegalArgumentException("Erro no mapa " + f.getName() + ": Mais de um BigFish encontrado! Posição duplicada em (" + x + ", " + y + ").");
                            }
                        ...
                        case 'S':
                            smallFishCount++;
                            if (smallFishCount > 1) {
                                throw new IllegalArgumentException("Erro no mapa " + f.getName() + ": Mais de um SmallFish encontrado! Posição duplicada em (" + x + ", " + y + ").");
                            }
            ...
            // Validação final: verificar se ambos os peixes estão presentes
            if (bigFishCount == 0) {
                throw new IllegalArgumentException("Erro no mapa " + f.getName() + ": BigFish (B) não encontrado! O mapa deve ter exatamente um BigFish.");
            }
            if (smallFishCount == 0) {
                throw new IllegalArgumentException("Erro no mapa " + f.getName() + ": SmallFish (S) não encontrado! O mapa deve ter exatamente um SmallFish.");
            }
